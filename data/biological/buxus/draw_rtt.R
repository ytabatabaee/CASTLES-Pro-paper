...
ggplot(r, aes(x=rtt, color=method, fill=method)) +
  ...
  geom_vline(data=mu, aes(xintercept=grp.mean, color=method),
             linetype="dashed")+
  geom_text(aes(color="CASTLES-Pro",y=3,x=0.286,label="avg.=0.286\nmax=0.549"),size=2.7)+
  geom_text(aes(color="CAML",y=4,x=0.308,label="avg.=0.308\nmax=0.529"),size=2.7)+
  theme(legend.position = c(.8,1), legend.direction = "vertical")
  #guides(color=guide_legend(nrow=2, byrow=TRUE))
ggsave("rtt_buxus_353.pdf",width=4,height=3.5)

corr=read.csv('buxus_corr_353.csv')
...
ggplot(aes(x=l1,y=l2,color=Branch.Type),data=corr[corr$l1 > 0 & corr$l2 > 0,])+
  ...
  coord_cartesian(xlim=c(10^-3.4,0.4),ylim=c(10^-3.4,0.4))+
  theme_classic()+
  theme(legend.position = c(.2,.8)) + 
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
ggsave("buxus_353_corr_r.pdf",width=4,height = 3.5)
