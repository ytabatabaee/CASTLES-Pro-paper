...
ggplot(r, aes(x=rtt, color=method, fill=method)) +
  ...
  geom_vline(data=mu, aes(xintercept=grp.mean, color=method),
             linetype="dashed")+
  geom_text(aes(color="CASTLES-Pro",y=2,x=1.685,label="avg.=1.685\nmax=2.524"),size=2.7)+
  geom_text(aes(color="CAML",y=3,x=1,label="avg.=0.905\nmax=1.237"),size=2.7)+
  theme(legend.position = c(.8,1), legend.direction = "vertical")
  #guides(color=guide_legend(nrow=2, byrow=TRUE))
ggsave("rtt_non_ribosomal.pdf",width=4,height=3.5)

corr=read.csv('non_ribosomal_corr.csv')
...
ggplot(aes(x=l1,y=l2,color=Branch.Type),data=corr)+
  ...
  coord_cartesian(xlim=c(10^-3,1.5),ylim=c(10^-3,1.5))+
  theme_classic()+
  theme(legend.position = c(.2,.8)) + 
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
ggsave("non_ribosomal_corr_r.pdf",width=4,height = 3.5)
...
ggplot(aes(x=l1,y=l2,color=Branch.Type),data=corr)+
  ...
  theme(legend.position = c(.2,.8)) + 
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
ggsave("non_ribosomal_corr_r_main.pdf",width=3.3,height = 3.5)
