...
ggplot(r, aes(x=rtt, color=method, fill=method)) +
  ...
  geom_vline(data=mu, aes(xintercept=grp.mean, color=method),
             linetype="dashed")+
  geom_text(aes(color="CASTLES-Pro",y=6,x=0.240,label="avg.=0.240\nmax=0.315"),size=2.7)+
  geom_text(aes(color="CAML",y=9,x=0.255,label="avg.=0.253\nmax=0.339"),size=2.7)+
  theme(legend.position = c(.8,1), legend.direction = "vertical")
  #guides(color=guide_legend(nrow=2, byrow=TRUE))
ggsave("rtt_mammals.pdf",width=4,height=3.5)

corr=read.csv('mammals_corr.csv')
