...
levels(r$method) = list("CASTLES-Pro" = "castles", "CAML" = "caml")
...
ggplot(r, aes(x=rtt, color=method, fill=method)) +
  ...
  geom_vline(data=mu, aes(xintercept=grp.mean, color=method),
             linetype="dashed")+
  geom_text(aes(color="CASTLES-Pro",y=6,x=0.203,label="avg.=0.203\nmax=0.288"),size=2.7)+
  geom_text(aes(color="CAML",y=9,x=0.231,label="avg.=0.231\nmax=0.314"),size=2.7)+
  theme(legend.position = c(.2,1), legend.direction = "vertical")
  #guides(color=guide_legend(nrow=2, byrow=TRUE))
ggsave("rtt_stiller.pdf",width=4,height=3.5)

corr=read.csv('stiller_corr.csv')
...
ggplot(aes(x=l1,y=l2,color=Branch.Type),data=corr)+
  ...
  coord_cartesian(xlim=c(10^-4.4,0.2),ylim=c(10^-4.4,0.2))+
  theme_classic()+
  theme(legend.position = c(.2,.8)) + 
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
ggsave("stiller_corr_r.pdf",width=4,height = 3.5)
