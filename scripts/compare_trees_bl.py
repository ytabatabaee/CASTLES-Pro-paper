...
def plot_correlations(df, name1, name2):
    ...
    sns.lmplot(data=df, x="log10(l1)", y="log10(l2)", palette='Dark2', hue="Branch Type",
               scatter_kws={'alpha': 0.1, 'linewidth': 0}, order=2, ci=0) #
    ...
    ax = plt.gca()
    ax.set_xlabel('log10 (CASTLES-Pro-2573 length)')
    ax.set_ylabel('log10 (CAML-353 length)')
...
def compare_bl(args):
    ...
    for node in t1.postorder_node_iter():
        ...
        if l2 < 0:
            neg_branches += 1
        #if l2 > 0 and (l1/l2 > 10):# or l1/l2 < 0.1):
        #    idx += 1
        #    continue
        #if l1 > 0 and math.log10(l1) < -5:
        #    idx += 1
        #    continue
