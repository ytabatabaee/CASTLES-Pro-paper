...
def read_ralphas(tree_path, rates_path, tns):
    t = dendropy.Tree.get(path=tree_path, schema='newick', taxon_namespace=tns)
    # read rate multipliers in pre-order traversal of the tree

    mu_rates = []
    for line in open(rates_path):
        mu_rates = line.split("\t")
        mu_rates = [float(mu) for mu in mu_rates[:-1]]
    # preorder_labels = [node.label for node in t.preorder_node_iter()]

    print(mu_rates)
    idx = 0
    for node in t.preorder_node_iter():
        node.edge.length = mu_rates[idx]
        node.label = None
        idx += 1
...
def castles(args):
    ...
    st_rates, rates_pre_order = read_ralphas(args.speciestree, args.ralphas, tns)

    length_diffs = dendropy.calculate.treecompare._get_length_diffs(st, st_rates)
    rates_post_order = [x for _, x in length_diffs]
    print(rates_post_order)
    ...
    for node in st.postorder_node_iter():
        node.edge.length = rates_post_order[idx]
        idx += 1'''
    ...
    for node in st.postorder_node_iter():
        if node.taxon is None:
            ...
            mu = ln_i
            break
        #if node.parent_node is not None and node.parent_node.parent_node is not None and node.taxon is None:

    print('global mu:', global_mu)

    # assign mutation rates to all nodes
    idx = 0
    for node in st.postorder_node_iter():
        node.edge.length = global_mu * rates_post_order[idx]
        idx += 1

    idx = 0
    for node in st.postorder_node_iter():
        rate = rates_post_order[idx]
        idx += 1
        if node.taxon is not None:
            ...
            mu1 = node.edge.length
            print('ln_i, mu1, ln_i/mu1', ln_i, mu1, ln_i/mu1)

            delta = safe_div(lm_i - ln_i, ln_i) if lm_i > ln_i else 1e-03
            l_est = 1/6 * (3 * delta + np.sqrt(3 * delta * (4 + 3 * delta))) * mu1#ln_i
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--ralphas", type=str, required=True,
                        help="Rate multipliers in pre-order traversal")
